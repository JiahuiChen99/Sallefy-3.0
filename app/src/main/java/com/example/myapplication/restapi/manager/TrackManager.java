...
public class TrackManager {
    ...
    public synchronized void getLikedTracks(final TrackCallback trackCallback) {
        ...
        call.enqueue(new Callback<List<Track>>() {
            ...
            public void onResponse(Call<List<Track>> call, Response<List<Track>> response) {
                ...
                if (response.isSuccessful()) {
                    trackCallback.onLikedTracksReceived(response.body());
                } else {
                    Log.d(TAG, "Error Not Successful: " + code);
                    trackCallback.onNoLikedTracks(new Throwable("ERROR " + code + ", " + response.raw().message()));
                }
            }
    ...
    public synchronized void likeTrack(Integer id, final TrackCallback trackCallback){
        UserToken userToken = Sesion.getInstance(mContext).getUserToken();

        Call<Track> call = mTrackService.likeTrack(id, "Bearer " + userToken.getIdToken());
        call.enqueue(new Callback<Track>() {
            @Override
            public void onResponse(Call<Track> call, Response<Track> response) {
                int code = response.code();
                ...
                if(response.isSuccessful()){
                    trackCallback.onTrackLiked(response.body());
                }else{
    ...
    public synchronized void isLiked(Integer id, final TrackCallback trackCallback){
        UserToken userToken = Sesion.getInstance(mContext).getUserToken();

        Call<Track> call = mTrackService.likedTrack(id, "Bearer " + userToken.getIdToken());
        call.enqueue(new Callback<Track>() {
            @Override
            public void onResponse(Call<Track> call, Response<Track> response) {
                int code = response.code();
